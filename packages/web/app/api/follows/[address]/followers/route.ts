...
const MOCK_USERS = Array.from({ length: 45 }, (_, i) => {
  const index = i + 1;
  // Generate a valid-looking Stellar address (G + char + X's + index)
...
export async function GET(
  ...
  try {
    const res = await fetch(`${indexerUrl}/api/follows/${address}/followers?limit=${limit}&offset=${offset}`, {
      next: { revalidate: 0 }
    });
    
    if (res.ok) {
      const data = await res.json();
      // Since indexer returns followers as string[], we enrich them with usernames
    ...
    // Indexer not running/reachable, fallback to mock data
  }

  // Filter out the owner address from the list of followers
